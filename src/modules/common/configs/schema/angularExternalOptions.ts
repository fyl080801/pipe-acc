      ...
      function($scope, $http, $interpolate, $filter, sfSelect) {
        ...
        var processOptions = function(optionSource, data, current) {
          ...
          for (var i = 0; i < scope.form.options.length; i++) {
            if (
              typeof scope.form.options[i].value !== 'undefined' &&
              current === scope.form.options[i].value
            ) {
        ...
        var loadOptions = function(optionSource, newValue?) {
          if (
            ...
            typeof scope.externalOptions[optionSource] === 'object'
          ) {
            return;
          } else {
            scope.currentSource = optionSource;
          ...
          if (typeof scope.externalOptions[optionSource] === 'object') {
            processOptions(
              optionSource,
              scope.externalOptions[optionSource],
              current
            );
            return;
          }

          $http.get(optionSource, { responseType: 'json' }).then(
            function(res, status) {
        ...
        if (!!scope.form.optionData) {
          scope.$parent
            .evalExpr('this')
            .$watchCollection(scope.form.optionData, function(
              newOptions,
              oldOptions
            ) {
              var options = {};
              if (angular.isArray(newOptions)) {
                options = angular.isString(newOptions[0])
                  ? { enum: newOptions }
                  : { titleMap: newOptions };
              }
              processOptions(
                'data:' + scope.form.optionData,
                options,
                scope.form.selectedOption
              );
            });
        } else if (scope.form.parameters && scope.form.parameters.length) {
          for (var i: any = 0; i < scope.form.parameters.length; i++) {
            if (angular.isDefined(scope.form.parameters[i])) {
              scope.$watch(scope.form.parameters[i][1], function(
                newValue,
                oldValue
              ) {
...
boot.directive('externalOptions', externalOptions);

boot
  .filter('_externalOptionUriField', [
    '$injector',
    '$filter',
...
boot.directive('destroyHiddenData', [
  'sfSelect',
  function(sfSelect) {
    return {
      link: function(scope: any, element, attrs) {
        ...
        scope.$on('$destroy', function() {
          if (
            typeof scope.form.preserveOnDestroy === 'object' &&
            scope.form.preserveOnDestroy.condition
          ) {
...
boot.directive('oyInline', [
  ...
  function(schemaForm, sfValidator, sfPath, sfSelect) {
    return {
      ...
      link: function(scope: any, element, attrs, ngModel: any) {
        var useKey = sfPath.stringify(scope.schemaValidate.key),
          schema: any = {},
          title =
            scope.schemaValidate.title ||
            scope.schemaValidate.key.join('.') ||
            '';
        ...
        scope.$watchCollection('model' + useKey, function(newVal, oldVal) {
          if (ngModel.$validate) {
            ngModel.$validate();
            if (ngModel.$invalid) {
              // The field must be made dirty so the error message is displayed
              ngModel.$dirty = true;
